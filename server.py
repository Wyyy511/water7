...
import json, math, os, tempfile, uuid, sys, shutil
...
app=FastAPI(title='WaterPulse AI Agent API', version='9.1')
...
@app.get('/api/health')
def health():
    return {'status':'ok','app':'WaterPulse AI Agent','version':'9.1','deployment_layout':'server-self-healing','deepseek_configured':deepseek_configured(),'deepseek_model':deepseek_model(),'field_contract':contract_summary(),'timestamp':now_iso()}
...
def _auto_analyze(sid: str, sess: dict[str, Any], *, requested_scenario: str|None=None,
    ...
    trace[-1]['detail']='当前风险计算完成；结果已记录，可用于后续比较。'
    ...
    if requested_scenario:
        ...
        if sres.get('data') is not None:
            ...
            text,meta=analyze_tool_result(user_question='请用普通用户能听懂的话解释这种情况下相对当前情况发生了什么变化、对供应意味着什么，以及建议怎么做。',
                                          baseline=sess['baseline'], scenario=sess['scenario'], validation=sess.get('validation'), fallback=fallback)
            return {'status':'success','message':text,
                    'actions':[_action('查看当前分析结果','show_current'),_action('换一种情况','scenario_menu'),_action('导出完整报告','export_report')],
    ...
    text,meta=analyze_tool_result(user_question='请用普通用户能听懂的话解释当前风险结果、主要风险来源、重点供应地和管理建议。',
...
def agent_status():
    return {'status':'ok','agent_version':'9.1-user-first-deepseek',
...
async def agent_message(
    ...
    if action=='run_scenario':
        ...
        return {'session_id':sid,'status':'success','message':text,
                'actions':[_action('换一种情况','scenario_menu'),_action('导出完整报告','export_report')],'trace':trace,'ai':meta,
    ...
    if action=='show_current' and sess.get('baseline'):
        ...
        return {'session_id':sid,'status':'success','message':text,
                'actions':[_action('看看不同情况下会怎样','scenario_menu'),_action('导出报告','export_report')], 'trace':trace,'ai':meta,
    ...
    if sess.get('scenario') and msg:
        ...
        ans,meta=analyze_tool_result(user_question=msg, baseline=sess.get('baseline'), scenario=sess.get('scenario'),
                                     validation=sess.get('validation'), fallback=fallback)
        return {'session_id':sid,'status':'success','message':ans,'actions':[_action('换一种情况','scenario_menu'),_action('导出完整报告','export_report')],'trace':trace,'ai':meta}
    if sess.get('baseline') and msg:
        ...
        return {'session_id':sid,'status':'success','message':ans,'actions':[_action('看看不同情况下会怎样','scenario_menu'),_action('导出报告','export_report')],'trace':trace,'ai':meta}
